"""The structured outputs the two agents must produce.

These models are bound to the tasks via `output_json=`, so their `Field(description=...)`
text is not documentation — it is prompt. It is the only calibration signal the model
gets about what a 4 means versus a 7. Tightening scoring behavior is done here.
"""
...
# Built from ESCALATION_THRESHOLD so the severity floor cannot drift away from the
# threshold the resolver is told to escalate on.
_FRUSTRATION_SCORE_DESCRIPTION = (
    ...
    f"duplicate charge, or a complete outage, the score is never below {ESCALATION_THRESHOLD} no matter how calm the "
...
class TicketTriageResult(BaseModel):
    ...
    ticket_id: str = Field(
        ..., description="The unique identification key of the support ticket."
    )
    category: str = Field(
        ...,
        description="Classification category (e.g., Technical, Billing, General Query).",
    )
    frustration_score: int = Field(
        ..., ge=1, le=10, description=_FRUSTRATION_SCORE_DESCRIPTION
    )
    ...
    summary: str = Field(
        ...,
        description="A concise structural summary of the issue reported by the customer.",
    )
...
class TechnicalResolutionResult(BaseModel):
    ...
    resolution_status: ResolutionStatus = Field(
        ...,
        description="Outcome status: either 'resolved_draft' or 'escalated_to_human'.",
    )
    email_response_template: Optional[str] = Field(
        None, description="The drafted customer reply if resolved. None if escalated."
    )
    internal_escalation_notes: Optional[str] = Field(
        None, description="Detailed technical handover notes if escalated."
    )

    @property
    def escalated(self) -> bool:
        return self.resolution_status is ResolutionStatus.ESCALATED_TO_HUMAN
